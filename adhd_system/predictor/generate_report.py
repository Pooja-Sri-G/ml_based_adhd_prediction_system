...
ON_TIME_THRESHOLD_MS = 500   # ±500 ms is considered "On Time"
...
def _chart_time_game(time_game_data):
    ...
    n       = len(rounds)
    targets = [r['target_ms'] / 1000 for r in rounds]
    ...
    bar_colors = ['#EF4444' if e < -ON_TIME_THRESHOLD_MS else ('#22C55E' if abs(e) <= ON_TIME_THRESHOLD_MS else '#F97316') for e in errors]
    ax2.bar(x, errors, color=bar_colors, alpha=0.85)
    ax2.axhline(0, color='#1E293B', linewidth=1)
    ax2.axhline( ON_TIME_THRESHOLD_MS, color='#22C55E', linewidth=1, linestyle=':', alpha=0.6)
    ax2.axhline(-ON_TIME_THRESHOLD_MS, color='#22C55E', linewidth=1, linestyle=':', alpha=0.6)
    ax2.set_xticks(x)
    ...
    early_count = sum(1 for e in errors if e < -ON_TIME_THRESHOLD_MS)
    ...
    patches = [
        ...
        mpatches.Patch(color="#22C55E", label="On time (±500 ms)"),
...
def _symptom_interpretation(user_data, prediction, game_data, time_game_data):
    ...
    if rounds:
        ...
        early_count = sum(1 for e in errors if e < -ON_TIME_THRESHOLD_MS)
        late_count  = sum(1 for e in errors if e >  ON_TIME_THRESHOLD_MS)
        on_time_count = len(rounds) - early_count - late_count
        ...
        time_parts.append(
            f"Out of {len(rounds)} rounds: {on_time_count} on time (within ±500 ms), "
            f"{early_count} too early, {late_count} too late."
        )
...
def generate_adhd_report(user_data: dict, game_data: dict, prediction: int,
    ...
    if time_performed:
        ...
        time_early_count = sum(1 for e in time_errors if e < -ON_TIME_THRESHOLD_MS)
    ...
    if rounds:
        ...
        for i, r in enumerate(rounds):
            ...
            if abs(dev) <= ON_TIME_THRESHOLD_MS:
                direc = "On Time "
            elif dev < 0:
                direc = "Too Early "
            else:
        ...
        story.append(Paragraph(
            ...
            f"Early releases: {early_count}/{len(rounds)} · Late releases: {late_count}/{len(rounds)} · "
            "On time = within ±500 ms · Red bars = too early (ADHD-associated) · Orange = too late.",
            caption_style))
    else:
        story.append(Paragraph(
